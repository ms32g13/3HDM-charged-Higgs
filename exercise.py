...
massrange = fl.massrange# charged Higgs ranged values from eehh.py
...
#   plt.show()
   print('--------------------------------------------------')
   print(len(A),len(B),BRCBfinal)
   print('CCCc',fl.eeHH_event() * 0.8**2 * 0.7**2 /np.sqrt(fl.backgroundtagging()* 0.1**2))
   ...
   print('---------------------------------------------------')
   print(fl.eeHH_event() * np.array(BRCBfinal)**2 * 0.7**2 )
...
#      np.sqrt(fl.backgroundtagging() * 0.1**2))
   ...
   Contoursignal0 = plt.contour(A,B, \
         np.resize(fl.eeHH_event() * np.array(BRCBfinal)**2 * 0.7**2 * fl.epsilon /np.sqrt(fl.backgroundtagging()),\
        len(fl.eeHH_event() * np.array(BRCBfinal)**2 * 0.7**2 * fl.epsilon /np.sqrt(fl.backgroundtagging()))).\
...
#############################   
   plt.figure()
   Contoursignal1 = plt.contour(A,B, \
        np.resize((fl.eeHHcbcb_2bsignal(BRCBfinal,BRCBfinal) + fl.eeHHcbcb_1bsignal(BRCBfinal,BRCBfinal) + \
                   fl.eeHHcbcs_2bsignal(BRCBfinal,BRCSfinal) + fl.eeHHcbcs_1bsignal(BRCBfinal,BRCSfinal) + \
              ...
              len((fl.eeHHcbcb_2bsignal(BRCBfinal,BRCBfinal) + fl.eeHHcbcb_1bsignal(BRCBfinal,BRCBfinal) + \
                   fl.eeHHcbcs_2bsignal(BRCBfinal,BRCSfinal) + fl.eeHHcbcs_1bsignal(BRCBfinal,BRCSfinal) + \
                   fl.eeHHcbcb_0bsignal(BRCBfinal,BRCBfinal) + fl.eeHHcbcs_0bsignal(BRCBfinal,BRCSfinal) + fl.eeHHcscs_0bsignal(BRCSfinal,BRCSfinal))* fl.epsilon / np.sqrt(fl.backgroundtagging()))).\
        reshape(len(B),len(A)),colors = ['black','royalblue','purple','darkgreen','brown','red','gray'])# ,levels = np.arange(1.0,6.0,1.0))
   plt.title('Significance of $H^{\pm}$ tagging in '+\
             ',$M_{H^{\pm}}$= '+ str(mhch) +' GeV')
   plt.xlabel(readlist[int(read1)])# x-axis label
   plt.ylabel(readlist[int(read2)])# y-axis label
   plt.grid(axis='y', linestyle='-', color='0.75') # show y-axis grid line
   plt.grid(axis='x', linestyle='-', color='0.75') # show x-axis grid line
   plt.colorbar(Contoursignal1)
   plt.savefig('significance_jet'+ str(mhch) +'.png')
   plt.show()
...
################################
   plt.figure()
   Contoursignal3 = plt.contour(A,B, \
        np.resize(fl.eeHH_event() * (np.array(BRCBfinal) + np.array(BRCSfinal)) * fl.epsilon / np.sqrt(fl.backgroundnotagging()),\
              len(fl.eeHH_event() * (np.array(BRCBfinal) + np.array(BRCSfinal)) * fl.epsilon / np.sqrt(fl.backgroundnotagging()))).reshape(len(B),len(A)),\
        cmap = 'brg')#,levels = np.arange(0.0,5.0,1.0))
   plt.colorbar(Contoursignal3)
   plt.title('Signicance BR($H^{\pm} \longrightarrow $ cb + cs),$M_{H^{\pm}}$= '+ str(mhch) +' GeV')#plot title
   plt.xlabel(readlist[int(read1)])# x-axis label
   plt.ylabel(readlist[int(read2)])# y-axis label
   plt.grid(axis='y', linestyle='-', color='0.75') # show y-axis grid line
   plt.grid(axis='x', linestyle='-', color='0.75') # show x-axis grid line
