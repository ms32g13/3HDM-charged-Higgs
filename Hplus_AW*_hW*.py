...
#some constants
ma = 80
print('mhch,mh,ma',mhch,mh,ma)
ka = (ma / mhch)**2    #Ma^2 / Mhch^2 = k_i or k_j
kw = (mw / mhch)**2    #Mw^2 / Mhch^2 = k_i or k_j
kh = (mh / mhch)**2    #Mh^2 / Mhch^2 = k_i or k_j
...
#        jac = (x1_max-x1_min)*(x2_max-x2_min)
        numer = (1.0 - x[0] ) * (1.0 - x[1] ) - ka
        denom = (1.0 - x[0] - x[1] - ka + kw)**2 + kw * gam_2  
        return  numer/denom #* jac
integ = vegas.Integrator([[x1_min, x1_max], [x2_min,x2_max]])
FHpaw = integ( Faw,nitn=10, neval=5000) 
print(9.0 * gf**2 * mw**4 * mhch * FHpaw / (16 * PI**2) )
print(9.0 * gf**2 * mw**4 * mhch * g(ka,kw) / (16 * PI**2) )