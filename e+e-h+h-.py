...
def cross_section_eeww():#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""
Created on Fri Oct 26 12:05:15 2018
...
### INVARIANT VALUE (PHYSICS VALUE OF SM PARAMATERS)
mt = 171.2    # mass of top quark
mz = 95.03288904374106 #98.14954576223639# 101.59443179342345  #91.18    # mass of z boson
nf = 5.0      # origin 5
PI = math.pi  # pi number 
mw = 85.0   # mass of w boson
mtau = 1.7771 # mass of tau neutrino
gf = 0.0000116639 # fermi constant
pwz = 2.4952  # partial width of z boson
pww = 2.085  #  partial width of w boson 
alpha_weak = 1.0 / 137.0 # alpha for sigma_0 
#mhch = 130.00  # mass of charged higgs
ma = 70.00 # mass of Pseudoscalar 
e_b = 0.55 # b-tag efficiency ratio
e_HHchar = 0.82# charged_higgs selection efficiency
############
# CKM elements
...
vcb = 0.04
###########
massrange = np.arange(70.00,91.00,1.00)# charged Higgs ranged values
print('charH_mass:',massrange)
costhetaw = mw / mz                     # cos (weinberg angle : thetaw)
sinthetaw = math.sqrt(1 - (costhetaw) ** 2)      # sin(weinberg angle: thetaw)
#S = np.arange(180.00 , 219.00 , 10.00) ** 2  # Centre of Mass Energy ^ 2
########################################################
# DELPHI: (CENTRE OF MASS ENERGY= sqrt(S))^2
#S_tuple = (180.0**2,192.0**2,196.0**2,200.0**2,202.0**2,205.0**2,206.3**2,206.6**2)
#S_tuple = (1000.0**2,1020.0**2,1040.0**2,1060.0**2,1080.0**2,1100.0**2,1120.3**2,1140.6**2)
#S = np.array(S_tuple)
squared_s = np.arange(170.0,301.0,1.0)
S = (np.array(squared_s)**2)
#print(squared_s)
...
#        print('||||','num of event',k,'S',np.array(S_tuple)*len(massrange),'Luminosity',Lumin_h_array [k],Product_HH)
    ...
    return  print('e+e- > H+ H- signals:',eecharH,'events')#final charged Higgs signal events of range
#eeHH_event()
...
# e+e->ww total events bakcground
def eeww_event():
    summ_ww =[]
    for k in np.arange(0,len(S_tuple)):
...
#print('|| e+e- > w w background:',eeww_event(),'events')
#print('||||After multiply 0.7^2:',eeww_event() * 0.7**2,'events') #5666.2838797003815
#epsilon_background =   1855 / 5666.2838797003815 * (3389.4 / 2506.2) =  0.44274400022087607  
#################################
#NONSENSE
print('S/sqrt(B)?:',36.09908588611527 * 0.6658269896193771 / math.sqrt(5666.2838797003815 * 0.44274400022087607 ))
################################
############################################################
...
alpmhch = alpmz /(1.0 + (alpmz/(12.0 * PI)) * (33.0 - 2 * nf) * \
              np.log((np.asarray(massrange)**2)/(mz**2))) 
...
################################################################################### 
def boson_decay_exist():  #check bosonic decay of charged Higgs exist or not
    zero = []
    for i in massrange:                                                                                                                                                                                              
        if i > ma:
#           gaw = 0.1
